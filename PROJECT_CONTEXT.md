...
| 10.8 | Notificações locais reais (título/corpo contextual) | 🔄 | `ReminderBroadcastReceiver.kt`: `contextualBody()` gera texto por categoria + nome do pet. `ReminderScheduler` agora passa petId, petName, petPhotoPath, category, title no Intent. Schedule chamado no `NewReminderViewModel.saveReminder()` após insert/update. **Bug fix (2026-07-12):** `SCHEDULE_EXACT_ALARM` estava declarada com `android:maxSdkVersion="32"`, tornando-a invisível no Android 13+ — app não aparecia em Configurações → Alarmes e lembretes e `canScheduleExactAlarms()` retornava false silenciosamente. Corrigido: removido `maxSdkVersion`; adicionado diálogo de runtime em `NewReminderScreen.kt` que detecta permissão ausente e abre `Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM`. |
