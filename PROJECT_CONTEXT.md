...
| # | Item | Status | Notas |
|---|------|--------|-------|
| 13.1 | Modal customizado (não diálogo padrão Android) | 🔄 | `Dialog` Compose com Card arredondado + `feedback_erro.png`; ícone de lixeira na TopBar da tela de detalhe abre o modal |
| 13.2 | Mensagem com nome do pet | 🔄 | "Tem certeza que deseja remover **{nome}**? Todos os registros de saúde também serão excluídos permanentemente." |
| 13.3 | Botão "Cancelar" (neutro) e "Remover" (vermelho arredondado do app) | 🔄 | `OutlinedButton` neutro + `Button` vermelho (`error`) com `RoundedCornerShape(24.dp)`, tipografia Nunito do app |
